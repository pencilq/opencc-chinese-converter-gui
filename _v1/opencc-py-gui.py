...
class OpenCCGUI:
    ...
    def setup_ui(self):
        ...
        default_font = ("Cascadia Code", 10, "normal")
        try:
            self.root.option_add("*Font", default_font)
        except:
            fallback_font = ("微软雅黑", 10, "normal")
        ...
        # Title
        title_label = ttk.Label(main_frame, text="OpenCC 中文转换器", 
        ...
        text_input_frame = ttk.LabelFrame(main_frame, text="直接文本输入（可替代文件输入）", padding="10")
        text_input_frame.grid(row=2, column=0, columnspan=3, sticky="ew", pady=(0, 10))
        ...
        # Conversion mode section with multi-layer settings
        mode_frame = ttk.LabelFrame(main_frame, text="转换设置", padding="10")
        ...
        # Column selection section (for Excel files)
        self.column_frame = ttk.LabelFrame(main_frame, text="列选择（Excel文件）", padding="10")
        self.column_frame.grid(row=4, column=0, columnspan=3, sticky="ew", pady=(0, 10))
        
        # Dropdown for column selection
        ttk.Label(self.column_frame, text="选择列:").grid(row=0, column=0, sticky=tk.W, padx=(0, 5))
        self.column_var = tk.StringVar()
        self.column_dropdown = ttk.Combobox(self.column_frame, textvariable=self.column_var, state="readonly")
        self.column_dropdown.grid(row=0, column=1, sticky="ew", padx=(0, 5))
        self.column_dropdown.bind('<<ComboboxSelected>>', self.on_column_selection_change)
        self.column_frame.columnconfigure(1, weight=1)
        
        # Preview section
        preview_frame = ttk.LabelFrame(main_frame, text="预览与复制区域", padding="10")
        ...
        ttk.Button(button_frame, text="清空", command=self.clear_all).grid(row=0, column=1)
    ...
    def update_column_selection(self):
        """Update column selection dropdown for Excel files"""
        if self.file_data is not None and hasattr(self.file_data, 'columns'):
            columns = ['选择一列...'] + self.file_data.columns.tolist()
            self.column_dropdown['values'] = columns
            self.column_dropdown.set(columns[0])
        else:
            self.column_dropdown['values'] = ['没有可用列']
            self.column_dropdown.set('没有可用列')
    
    def clear_column_selection(self):
        """Clear column selection for non-Excel files"""
        self.column_dropdown['values'] = ['不适用于此文件类型']
        self.column_dropdown.set('不适用于此文件类型')
    
    def get_selected_columns(self):
        """Get list of selected columns"""
        selected = self.column_var.get()
        if selected and selected not in ['选择一列...', '没有可用列', '不适用于此文件类型']:
            return [selected]
        return []
        ...
        try:
            ...
            if self.file_type.get() == 'excel':
                ...
                if not selected_cols:
                    ...
                    self.preview_text.insert(tk.END, f"请选择一列进行转换。\n\n可用列: {available_cols}")
                    return
                ...
                for col in selected_cols:
                    if col in converted_data.columns:
                        for idx in range(len(converted_data)):
                            value = converted_data.iloc[idx, converted_data.columns.get_loc(col)]
                            if pd.notna(value) and str(value) != 'nan' and str(value).strip():
                                converted_data.iloc[idx, converted_data.columns.get_loc(col)] = self.converter.convert(str(value))
                            
                            # Update progress
                            progress_percentage = int((idx + 1) / total_rows * 100)
                            progress_msg = f"正在转换列 '{col}': {idx+1}/{total_rows} ({progress_percentage}%)"
